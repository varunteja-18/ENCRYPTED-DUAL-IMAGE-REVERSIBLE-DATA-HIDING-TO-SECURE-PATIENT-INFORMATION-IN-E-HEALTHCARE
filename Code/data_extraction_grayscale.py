...
def readGrayScaleStegoImage():
    ...
    for i,j in zip(dem_grayscale.watermark_alpha_resized.flatten(),thresholded_watermark.flatten()):
        if i == j:
            flag = 1
        else :
            flag = 0
...
def func_BER():
    global ber
    ber = (sum(i^j for i,j in zip(dem_grayscale.watermark_alpha_resized.flatten(),thresholded_watermark.flatten())) * 100) / (dem_grayscale.count3 - dem_grayscale.count2)
    print('BER :',ber)
func_BER()
