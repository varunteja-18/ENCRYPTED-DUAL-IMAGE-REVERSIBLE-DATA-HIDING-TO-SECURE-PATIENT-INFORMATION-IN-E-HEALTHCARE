...
## Function to Create GUI for Selecting Stego Image - DICOM
def selectDICOMStegoGUI():
    ...
    def close_window():
        if (dropdownbox_input.current() != -1):
            ...
            messagebox.showerror("Error", "Selection of Dicom Stego Image CANNOT be Empty!!")
    ...
    def saveToFile():
        ...
        if not stego_image.strip():
            messagebox.showerror("Error", "Select DICOM stego image.")
            flag = 1
        ## Open a Text File in Write Mode and Write the Option Selected from the Drop Down Box to the File
        with open("DICOM Stego Image Selected.txt","w") as img_file:
    ...
    gui_window.title('Patient Details(Selection of Stego Image) - DICOM')
    gui_window.geometry("600x400")

    ## Create the Input Fields
    ## Drop Down Box Creation
    dropdownbox_label = Label(gui_window, text='DICOM Stego Image :' ,font=("Times New Roman", 15))
    dropdownbox_options = ['dicom_stego_image.dcm'] # create a list of options
